...
Four roles - BDA, Sales Lead, Leadership and Admin - divide the pipeline,
chosen from the CRM dropdown on a user's Access Rights page. A BDA works only
the opportunities they own. Nobody below Admin can delete anything; records
are archived or marked lost instead. The Sales dropdown beside it is read-only
and follows the role, so the two cannot be set to disagree.
""",
    ...
    # Makes the Sales dropdown on the Access Rights page read-only. It has to
    # be done in the browser: that page is generated by the res_user_group_ids
    # widget, not by an arch a view could inherit.
    'assets': {
        'web.assets_backend': [
            'crm_consulting_customization/static/src/res_user_group_ids_field.js',
            'crm_consulting_customization/static/src/res_user_group_ids_field.xml',
        ],
    },
