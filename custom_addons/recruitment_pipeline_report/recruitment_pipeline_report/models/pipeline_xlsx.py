...
from .pipeline_constants import PRIMARY_HEADERS, SECONDARY_HEADERS, MEASURE_FIELDS


def build_pipeline_xlsx(rows):
    ...
    for row_idx, row in enumerate(rows, start=2):
        ...
        for col, field_name in enumerate(MEASURE_FIELDS, start=identity_count):
            worksheet.write(row_idx, col, row.get(field_name, 0) or 0, num_fmt)
